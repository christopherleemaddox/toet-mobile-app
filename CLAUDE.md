...
Christopher runs this app on his own phone every day. It's deployed on Netlify at **bibletruthcards.netlify.app** (password-protected in Netlify's visitor-access settings — intentional, see below). Deploys have been **manual** (drag-and-drop into the Netlify dashboard); moving to git-based auto-deploy is in progress as of 2026-08-07 — `netlify.toml` (`publish = "Live Build"`) is committed so Netlify knows what to serve once the site is linked to the `github.com/christopherleemaddox/toet-mobile-app` repo in Netlify's dashboard. That linking step has to happen in Netlify's UI (OAuth to GitHub) — not something that can be done from a terminal.
